...
for (i in request_cancer) {
  ...
  df<-subset(clinical,select =c(submitter_id,vital_status,days_to_death,days_to_last_follow_up,Treg_1C_gl,Treg_1C_gc,age_at_index,gender,Treg_1C_gl_2,Treg_1C_gc_2,Treg_1C_gl_3,Treg_1C_gc_3,ann_arbor_clinical_stage))
  ...
  ggcoxzph(test.ph)
  newrescat<-cbind(res.cat,df$age_at_index,df$gender,df$ann_arbor_clinical_stage)
  ...
  res.cox_new<-coxph(Surv(Time,events) ~ Treg_1C_gc + Age+gender+stage ,data=newrescat)
  summary(res.cox_new)
