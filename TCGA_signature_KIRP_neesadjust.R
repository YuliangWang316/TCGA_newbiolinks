...
for (i in request_cancer) {
  ...
  res.cox_new<-coxph(Surv(Time,events) ~ Treg_1C_gc + gender+stage ,data=newrescat)
  summary(res.cox_new)
  ...
  ggcoxzph(test.ph_new)
   ...
   for (j in 1:length(rownames(newrescat))) {
      if(newrescat$stage[j] == "Stage II"){
       newrescat$stage_new[j]<-0
     }else if(newrescat$stage[j] == "Stage III"){
  ...
  ggadjustedcurves(res.cox_new_new,data = newrescat,variable = "Treg_1C_gc",risk.table = TRUE,pval = TRUE)
}
